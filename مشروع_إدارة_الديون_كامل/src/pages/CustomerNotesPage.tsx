...
export default function CustomerNotesPage() {
  ...
  async function loadMerchants() {
    ...
    try {
      // استخدام Edge Function الجديد للحصول على قائمة شاملة بالتجار
      const { data, error } = await supabase.functions.invoke('get-customer-merchants', {
        body: { customer_phone: profile.phone_number }
      });
      ...
      if (data?.data?.merchants) {
        // استخدام البيانات المباشرة من Edge Function
        const merchantsList = data.data.merchants.map((merchant: any) => ({
          merchant_id: merchant.merchant_id,
          merchant_name: merchant.merchant_name,
          merchant_phone: merchant.merchant_phone,
          total_debts: merchant.total_debts,
          total_amount: merchant.total_amount,
          paid_amount: merchant.paid_amount,
          remaining_amount: merchant.remaining_amount,
          last_transaction_date: merchant.last_transaction_date
        }));
        
        setMerchants(merchantsList);
        console.log('تم تحميل التجار:', merchantsList.length);
      }
    } catch (error) {
      console.error('Error loading merchants:', error);
      toast.error('حدث خطأ في تحميل التجار');
    }
    setLoading(false);
  }
